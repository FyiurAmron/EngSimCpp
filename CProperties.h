...
public:
    ...
    CProperty( double *pointer, std::wstring name ) {
        indexInAllProps = -1;
        Set( pointer, name );
    }

    CProperty( double *pointer, std::wstring name, char shortcut ) {
        indexInAllProps = -1;
        Set( pointer, name, shortcut );
    }

    CProperty( double *pointer, std::wstring name, char shortcut, double sliderMin, double sliderMax ) {
        indexInAllProps = -1;
        Set( pointer, name, shortcut, sliderMin, sliderMax );
    }

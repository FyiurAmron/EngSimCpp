...
class NeuralNetwork
{
	void AddErrorSample(double e){error_sum+=e*e; error_den++;}
public:
	...
	static double tansigD(double x){	return 4*exp(-2*x)/(1+exp(-2*x))/(1+exp(-2*x));}
	static double purelin(double x){ return x;}
	static double purelinD(double x){ return 1;}

	void ResetError(){error_sum=0;error_den=0;}	//zerowanie bledu uczenia
	double GetError(){return error_sum/(double)error_den;}	//obliczanie bledu
};
